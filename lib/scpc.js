...
async function generateToken(agent) {
  const tokenPage = await agent.get(
    `${SCPC_URL}/rechercherParReferenceCadastrale.do`
  );
  const [, csrfToken] = tokenPage.text.match(
    /CSRF_TOKEN=((?:[\dA-Z]{4}-){7}[\dA-Z]{4})/
  );
...
function getPage(page, params, agent) {
  return agent.get(
    `${SCPC_URL}/${page}.do?CSRF_TOKEN=${agent.csrfToken}&${params}`
  );
}
...
async function fetchExtraitPlanCadastral(params) {
  REQUIRED_PARAMS.forEach((requiredParam) => {
    if (!(requiredParam in params)) {
      throw createError(
        400,
        `Le paramètre '${requiredParam}' est obligatoire.`
      );
    }
  });
  ...
  const searchPage = await sendForm(
    'rechercherParReferenceCadastrale',
    {
  ...
  const displayMapPage = await getPage(
    'afficherCarteParcelle',
    displayMapQuery,
    agent
  );

  // Extraction du centre de la parcelle
  const displayMapPageExtraction = displayMapPage.text.match(
    /new Point\((.*),(.*)\)/
  );
  ...
  const x = params.x
    ...
    : Number.parseFloat(displayMapPageExtraction[1]);

  const y = params.y
    ? Number.parseFloat(params.y)
    : Number.parseFloat(displayMapPageExtraction[2]);
  ...
  const { xMin, xMax, yMin, yMax } = computeBbox({
    x,
    y,
    taille,
    orientation,
  ...
  const finalResult = await sendForm(
    'imprimerExtraitCadastral',
    finalResultQuery,
    agent
  ).buffer();

  if (finalResult.type !== 'application/pdf') {
    throw createError(500, 'Impossible de récupérer le document', {
      expose: true,
    });
  }
